...
exports.updateBudget = (req, res) => {
	...
	let option1 = { budget: parseFloat(req.body.budget) };
	let option2 = { where: { id: req_user.id } };
	req_user.budget = parseFloat(req.body.budget);
	...
	uploadImage(req, res, (err) => {
		if (err) {
			...
			if (req.body.user !== undefined && req.body.user !== null) {
				...
				if (req_user.budget != data.budget) {
					user.budget = parseFloat(data.budget);
				}
...
exports.findAllUser = (req, res) => {
	...
	if (req.body.page !== undefined && req.body.page !== null && req.body.page == "auth") {
		...
		options = {
			include: [{ model: Activity }, { model: Order, include: [{ model: Products }] }], order: [
