...
class Profile extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            budget: { value: user.budget === undefined ? "2000" : user.budget, err: false, helperText: '' },
    ...
    _handlechange = (name, text) => {
        ...
        if (name == 'budget') {
            errors = this._validateBudget(errors);
        }
    ...
    submitForm = async () => {
        ...
        errors = this._validateMobile(errors); errors = this._validateBudget(errors);
        let err_check = false;
        for (x in errors) {
        ...
        if (err_check === false) {
            ...
            if (errors.budget.value != "") {
                user.budget = errors.budget.value;
            }
            ...
            <Modal visible={this.props.visibile} animationType={'slide'} onShow={() => { this._reset() }} onRequestClose={() => { console.log("Modal close") }}>
                ...
                <Content>
                    <List style={{ padding: 10 }}>
                        ...
                        <ListItem style={fstyles.formContainer}>
                            <Left style={fstyles.formLeft}><Label style={gstyles.inputlabel}>Budget Amount (Rs.) :</Label></Left>
                            <Body style={fstyles.formBody}>
                                <Item regular style={gstyles.ouritem}>
                                    <Input keyboardType="numeric" maxLength={10} value={this.state.budget.value === null ? '2000' : this.state.budget.value.toString()} onChangeText={(text) => this._handlechange('budget', text)} />
                                </Item>
